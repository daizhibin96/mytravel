...
public class AvailibleOrderActivity extends BaseActivity {
	...
	private void initOrder() {
		// TODO Auto-generated method stub
//		OrderPageService.findHotel("东莞", AvailibleOrderActivity.this);
//		OrderPageService.findPoint("东莞", AvailibleOrderActivity.this);
		...
		public void onReceive(Context arg0, Intent arg1) {
			...
			if (intentAction.equals("location.hotel")) {
				...
				for (int i = 0; i < hotelList.size(); i++) {
					String temp=hotelList.get(i).getIntime();
					String temp1=temp.replaceAll("年", "-");
					String s1=temp1.replaceAll("月", "-");
					String s2=s1.replaceAll("日", "");
					Log.i("order55time....",s2);
					if (((NowTime.compareTo(s2)<0)||(NowTime.equals("s2")))&& (hotelList.get(i).getPay().equals("已支付"))) {
						Order order55 = new Order(hotelList.get(i).getId(),"images/hotel.png", hotelList
								.get(i).getHotel(), hotelList.get(i).getIntime(),
								hotelList.get(i).getPrice(), 1);
			...
			if (intentAction.equals("location.point")) {
				...
				for(int i = 0;i < pointList.size(); i++){
					...
					String s2=s1.replaceAll("日", "");
					Log.i("order55time....",s2);
					if(((NowTime.compareTo(s2)<0)||(NowTime.equals("s2")))&&
							(pointList.get(i).getPay().equals("已支付"))){
			...
			OrderAdapter adapter = new OrderAdapter(AvailibleOrderActivity.this,
							...
							if (order.getOrdertype() == 1) {
								for(int i = 0;i < hotelList.size(); i++){
									if(hotelList.get(i).getId()==(order.getId())){
										...
										Intent intent1 = new Intent(AvailibleOrderActivity.this,
	...
	class CompareOrderTime implements Comparator<Order>{
		...
		public int compare(Order p1, Order p2) {
			// TODO Auto-generated method stub
			return p1.getOrdertime().compareTo(p2.getOrdertime());
		}
	}
